...
import com.hyp.learn.blog.business.enums.LinkSourceEnum;
...
import com.hyp.learn.blog.business.srvice.SysLinkService;
import com.hyp.learn.blog.business.util.LinksUtil;
import com.hyp.learn.blog.business.vo.LinkConditionVO;
import com.hyp.learn.blog.framework.exception.PxLinkException;
import com.hyp.learn.blog.persistence.beans.SysLink;
import com.hyp.learn.blog.persistence.mapper.SysLinkMapper;
...
public class SysLinkServiceImpl implements SysLinkService {
    ...
    public boolean autoLink(Link link) throws PxLinkException {
        String url = link.getUrl();
        Link bo = getOneByUrl(url);
        if (bo != null) {
            throw new PxLinkException("本站已经添加过贵站的链接！");
        }
        ...
        if (!(LinksUtil.hasLinkByHtml(url, domain))
            ...
            throw new PxLinkException("贵站暂未添加本站友情链接！请先添加本站友链后重新提交申请！");
        }
