...
import com.hyp.learn.blog.business.dto.PxCommentDTO;
...
import com.hyp.learn.blog.persistence.mapper.PxCommentMapper;
import com.hyp.learn.blog.utils.*;
...
@Slf4j
@Service
public class PxCommentServiceImpl implements PxCommentService {

    @Autowired
    private PxCommentMapper pxCommentMapper;
    ...
    public PageInfo<Comment> findPageBreakByCondition(CommentConditionVO vo) {
        ...
        List<PxComment> list = pxCommentMapper.findPageBreakByCondition(vo);
        List<Comment> boList = this.getComments(list);
        if (boList == null) {
            return null;
        }
        PageInfo bean = new PageInfo<PxComment>(list);
        bean.setList(boList);
        return bean;
    }
    ...
    private List<PxCommentDTO> convert2DTO(List<Comment> list) {
        if (CollectionUtils.isEmpty(list)) {
            return null;
        }
        List<PxCommentDTO> dtoList = new LinkedList<>();
        for (Comment comment : list) {
            PxCommentDTO dto = BeanConvertUtil.doConvert(comment, PxCommentDTO.class);
            dto.setParentDTO(BeanConvertUtil.doConvert(comment.getParent(), PxCommentDTO.class));
    ...
    public void commentForAdmin(Comment comment) throws PxCommentException {
    ...
    @Override
    @RedisCache(flush = true)
    public Comment comment(Comment comment) throws PxCommentException {
        ...
        if (!anonymous && !SessionUtil.isLogin()) {
            throw new PxCommentException("站长已关闭匿名评论，请先登录！");
        }
    ...
    private void filterContent(Comment comment) {
        ...
        if (StringUtils.isEmpty(content) || "\n".equals(content)) {
            throw new PxCommentException("说点什么吧");
        }
        // 过滤非法属性和无用的空标签
        if (!XssKillerUtil.isValid(content) || !XssKillerUtil.isValid(comment.getAvatar())) {
            throw new PxCommentException("请不要使用特殊标签");
        }
    ...
    private void setCurrentAnonymousUserInfo(Comment comment) {
        if (StringUtils.isEmpty(comment.getNickname())) {
            throw new PxCommentException("必须输入昵称");
        }
    ...
    private List<Comment> getComments(List<PxComment> list) {
        if (CollectionUtils.isEmpty(list)) {
            return null;
        }
        List<Comment> boList = new ArrayList<>();
        for (PxComment pxComment : list) {
            boList.add(new Comment(pxComment));
        }
        return boList;
    }
    ...
    public void doSupport(Long id) {
        ...
        if (redisTemplate.hasKey(key)) {
            throw new PxCommentException("一个小时只能点一次赞哈~");
        }
        pxCommentMapper.doSupport(id);
    ...
    public void doOppose(Long id) {
        ...
        ValueOperations<String, Object> operations = redisTemplate.opsForValue();
        if (redisTemplate.hasKey(key)) {
            throw new PxCommentException("一个小时只能踩一次哈~又没什么深仇大恨");
        }
        pxCommentMapper.doOppose(id);
        operations.set(key, id, 1, TimeUnit.HOURS);
    }
    ...
    public Comment insert(Comment entity) {
        ...
        pxCommentMapper.insertSelective(entity.getPxComment());
        return entity;
    }
    ...
    public boolean removeByPrimaryKey(Long primaryKey) {
        return pxCommentMapper.deleteByPrimaryKey(primaryKey) > 0;
    }
    ...
    public boolean updateSelective(Comment entity) {
        Assert.notNull(entity, "Comment不可为空！");
        entity.setUpdateTime(new Date());
        return pxCommentMapper.updateByPrimaryKeySelective(entity.getPxComment()) > 0;
    }

    @Override
    public Comment getByPrimaryKey(Long primaryKey) {
        Assert.notNull(primaryKey, "PrimaryKey不可为空！");
        PxComment entity = pxCommentMapper.getById(primaryKey);
