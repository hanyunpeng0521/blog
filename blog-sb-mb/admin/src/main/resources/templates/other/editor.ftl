                    ...
                    <div class="col-md-8 col-sm-8 col-xs-12">
                        <div id="editor">
                            ...
                            <ul>
                                ...
                                <li>uploadType: 当前上传文件的场景类型，<strong>最好根据实际业务取名</strong>，它会决定最终上传完成后的文件路径，比如在商品信息管理页中指定了uploadType
                                    = goods,那么最终上传完成后的文件路径就是：<code>oneblog/goods/{filename}.png</code>，默认为空
                                </li>
                                <li>customCss: 自定义的css，可以修改编辑器大小，默认为空。注：如果是修改高度，必须通过<code>max-height</code>参数修改，并且一定要加上：<code>"overflow-y":
                                        "scroll"</code>, <code>"height": "100%"</code>这两项配置，否则可能会使编辑器显示不正确
                                </li>
                            </ul>
                        </div>
                    </div>
