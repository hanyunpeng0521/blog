                ...
                <#if menus?? && menus?size gt 0>
                    <#list menus as item>
                        <#if item.nodes?? && item.nodes?size gt 0>
                            <li>
                                <a><i class="${item.icon!}"></i> ${item.name!}<span
                                    ...
                                    <#list item.nodes as node>
                                        <#if node.permission!>
                                            <@shiro.hasPermission name="${node.permission!}">
                                                <li>
                                                    <a href="${node.url!}" ${(item.external?? && item.external)?string('target="_blank"','')}><i
                                                                class="${node.icon!}"></i>${node.name!}</a></li>
                                            </@shiro.hasPermission>
                                        <#else>
                                            <li>
                                                <a href="${node.url!}" ${(item.external?? && item.external)?string('target="_blank"','')}><i
                        ...
                        <#else>
                            <li>
                                <a href="${item.url!}" ${(item.external?? && item.external)?string('target="_blank"','')}><i
